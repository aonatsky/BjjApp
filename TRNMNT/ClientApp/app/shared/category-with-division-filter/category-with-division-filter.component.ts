...
export class CategoryWithDivisionFilter implements OnInit {
  ...
  constructor(
    ...
    this.defaultOption = { label: this.translateService.instant('COMMON.ANY'), value: '' };
  ...
  private refreshWeightDivisionFilter(categoryId: string) {
    ...
    if (weightDivisions.length > 0) {
      ...
      if (this.currentFilterValue.weightDivisionId === '') {
        this.currentFilterValue.weightDivisionId = this.autoSelectFirstWeightDivision
          ? weightDivisions[0].weightDivisionId
          : '';
      }
    } else {
